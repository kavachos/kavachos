...
async function createTestHandlers(): Promise<{ handlers: Handlers; kavach: Kavach }> {
	...
	return {
		handlers: kavachAstro(kavach, { basePath: "/api/auth/kavach" }),
		kavach,
	};
}

async function callHandler(handler: Handlers[keyof Handlers], request: Request): Promise<Response> {
...
describe("Astro adapter", () => {
	...
	it("uses the configured basePath for agent create, list, and fetch", async () => {
		const { id } = await createTestAgent(handlers, { name: "astro-agent" });
		...
		expect(listBody.data[0]?.name).toBe("astro-agent");

		const getRes = await callHandler(
			handlers.GET,
			new Request(`${BASE_URL}/agents/${id}`, { method: "GET" }),
		);
	...
	it("authorizes by bearer token and denies after revoke", async () => {
		...
		const allowRes = await callHandler(
			handlers.POST,
		...
		const revokeRes = await callHandler(
			handlers.DELETE,
		...
		expect(getRes.status).toBe(200);
		expect(((await getRes.json()) as { data: { status: string } }).data.status).toBe("revoked");

		const denyRes = await callHandler(
			handlers.POST,
			new Request(`${BASE_URL}/authorize/token`, {
				method: "POST",
				headers: {

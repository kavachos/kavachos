...
describe("Nuxt adapter", () => {
	let handle: WebHandler;

	beforeEach(async () => {
		({ handle } = await createTestHandler());
	});

	it("routes create, list, and fetch flows through the configured basePath", async () => {
		const { id } = await createTestAgent(handle, { name: "nuxt-agent" });
		...
		expect(listBody.data[0]?.name).toBe("nuxt-agent");

		const getRes = await handle(new Request(`${BASE_URL}/agents/${id}`, { method: "GET" }));
	...
	it("authorizes by bearer token and denies after revoke", async () => {
		const { id, token } = await createTestAgent(handle);

		const allowRes = await handle(
		...
		const revokeRes = await handle(new Request(`${BASE_URL}/agents/${id}`, { method: "DELETE" }));
	...
	it("returns a BAD_REQUEST envelope for malformed JSON input", async () => {
		const res = await handle(
			new Request(`${BASE_URL}/agents`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },

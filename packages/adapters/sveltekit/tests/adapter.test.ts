...
async function createTestHandlers(): Promise<{ handlers: Handlers; kavach: Kavach }> {
	...
	return {
		handlers: kavachSvelteKit(kavach, { basePath: "/api/auth/kavach" }),
		kavach,
	};
}
...
describe("SvelteKit adapter", () => {
	...
	it("uses the configured basePath for create, list, and fetch", async () => {
		const { id } = await createTestAgent(handlers, { name: "sveltekit-agent" });
		...
		expect(listBody.data[0]?.name).toBe("sveltekit-agent");
